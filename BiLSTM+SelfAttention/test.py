...
class BiLSTMSelfAttention(nn.Module):
    def __init__(self, vocab_size,embedding_size ,hidden_size,num_heads,bidirectional,num_layers, num_classes,dropout ,embedding_weight=None):
        ...
        #     batch_first=True
        # )
        # self.pos = PositionalEncoding(embedding_size,55)

        self.lstm = nn.LSTM(embedding_size, hidden_size, num_layers,bidirectional=bidirectional,batch_first=True)
        
        self.attention = nn.MultiheadAttention(embed_dim=2*hidden_size,num_heads=num_heads,dropout=0.1,batch_first=True)
        # self.transformer_encoder = nn.TransformerEncoder(encoder_layer,num_layers=4)
        # self.ffn = PositionWiseFFN(2*hidden_size,2*hidden_size,2*hidden_size)
        # 层归一化
    ...
    def forward(self, X):
        ...
        # res = self.transformer_encoder(out2)
        out2,_ = self.attention(out1,out1,out1)
        # res = self.ffn(out2) 
        res = self.layer_norm(out1+out2)
        res = res.mean(dim=1)
        res = self.fc(self.dropout(res))
        return res
...
#导入模型
para = torch.load('/home/user_home/qiankunjie/project/First-exam/BiLSTM+SelfAttention/best_model_acc:0.78_f1:0.793_hidden:512_lr:0.001.pt')
model = BiLSTMSelfAttention(vocab_size,300,512,8,True,2,2,0.1).to(device)
model.load_state_dict(para)
#测试结果
f1, acc= evaluate_model(model, test_loader)
#输出结果
print("BiLSTMSelfAttention")
